...
exports.registerInboundRoutes = registerInboundRoutes;
...
function verifyTwilioSignature(authToken, url, params, signature) {
    ...
    const computed = (0, crypto_1.createHmac)('sha1', authToken).update(data).digest('base64');
    if (computed.length !== signature.length)
        return false;
    return (0, crypto_1.timingSafeEqual)(Buffer.from(computed), Buffer.from(signature));
}
/**
 * Verify Resend (Svix) webhook signature.
 * See: https://docs.svix.com/receiving/verifying-payloads/how-manual
 */
function verifyResendSignature(secret, msgId, timestamp, body, signatures) {
    // secret is base64-encoded after removing the "whsec_" prefix
    const secretBytes = Buffer.from(secret.replace(/^whsec_/, ''), 'base64');
    const toSign = `${msgId}.${timestamp}.${body}`;
    const computed = (0, crypto_1.createHmac)('sha256', secretBytes).update(toSign).digest('base64');
    ...
    return sigs.some(sig => {
        if (sig.length !== computed.length)
            return false;
        return (0, crypto_1.timingSafeEqual)(Buffer.from(computed), Buffer.from(sig));
    });
}
/** Build the full request URL for Twilio signature verification. */
function getFullUrl(req, ctx) {
...
function getTwilioAuthToken(ctx, channelName) {
    if (!ctx.configPath)
        return null;
    try {
        ...
        if (!ch)
            return null;
        return ch.auth_token || process.env.TWILIO_AUTH_TOKEN || null;
    }
    catch {
        return null;
    }
...
function getResendWebhookSecret(ctx, channelName) {
    if (!ctx.configPath)
        return null;
    try {
        const config = (0, parser_1.loadConfig)(ctx.configPath, { validate: false });
        ...
        if (!ch)
            return null;
        return ch.webhook_secret || null;
    }
    catch {
        return null;
    }
}
function registerInboundRoutes(app, ctx) {
    // POST /inbound/voice/:channel — Twilio Voice incoming call
    app.post('/inbound/voice/:channel', express_1.default.urlencoded({ extended: false }), (req, res) => {
        const channelName = req.params.channel;
        const channel = ctx.channels.get(channelName);
        if (!channel || !channel.handleIncomingCall) {
        ...
        try {
            ...
            const twiml = channel.handleIncomingCall(req.body, voiceConfig);
    ...
    app.post('/inbound/voice/:channel/recording', express_1.default.urlencoded({ extended: false }), async (req, res) => {
        const channelName = req.params.channel;
        const channel = ctx.channels.get(channelName);
        if (!channel || !channel.handleRecording) {
        ...
        try {
            const voiceConfig = ctx.findVoiceConfig?.(channelName);
            const twiml = await channel.handleRecording(req.body, voiceConfig);
            res.type('text/xml').send(twiml);
        }
        catch (err) {
    ...
    app.post('/inbound/voice/:channel/respond/:callSid', express_1.default.urlencoded({ extended: false }), (req, res) => {
        const { channel: channelName, callSid } = req.params;
        const channel = ctx.channels.get(channelName);
        if (!channel || !channel.handleRespond) {
            res.type('text/xml').send('<Response><Hangup/></Response>');
            return;
        }
        const authToken = getTwilioAuthToken(ctx, channelName);
        ...
        try {
            const voiceConfig = ctx.findVoiceConfig?.(channelName);
            const twiml = channel.handleRespond(callSid, voiceConfig);
            res.type('text/xml').send(twiml);
        }
        catch (err) {
    ...
    app.get('/inbound/voice/:channel/audio/:id', (req, res) => {
        ...
        const channel = ctx.channels.get(channelName);
        if (!channel || !channel.getAudio) {
            res.status(404).send('Not found');
            return;
        }
        const entry = channel.getAudio(id);
        if (!entry) {
    ...
    app.post('/inbound/twilio/:channel', express_1.default.urlencoded({ extended: false }), (req, res) => {
        const channelName = req.params.channel;
        const channel = ctx.channels.get(channelName);
        if (!channel || !channel.handleInbound) {
        ...
        try {
            channel.handleInbound(req.body);
    ...
    app.all('/inbound/endpoint/:channel', async (req, res) => {
        const channelName = req.params.channel;
        const channel = ctx.channels.get(channelName);
        if (!channel || !channel.handleRequest) {
        ...
        if (cfg.secret) {
            ...
            if (!provided || provided.length !== cfg.secret.length ||
                !(0, crypto_1.timingSafeEqual)(Buffer.from(provided), Buffer.from(cfg.secret))) {
        ...
        try {
            ...
            const { message, waitForResponse } = endpointChannel.handleRequest(req.body, query, headers, req.method);
            if (waitForResponse) {
    ...
    app.post('/inbound/resend/:channel', express_1.default.text({ type: '*/*', limit: '1mb' }), (req, res) => {
        const channelName = req.params.channel;
        const channel = ctx.channels.get(channelName);
        if (!channel || !channel.handleInbound) {
            res.status(404).json({ error: 'Not found' });
