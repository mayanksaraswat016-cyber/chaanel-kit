 ...
 */
async function aiRoute(message, services, channelConfig, settings) {
    const aiConfig = channelConfig.ai_routing;
    if (!aiConfig)
        return { serviceName: null, error: 'No ai_routing config' };
...
async function callOpenAI(systemPrompt, userMessage, model, apiKey) {
    if (!apiKey)
        return { serviceName: null, error: 'OpenAI API key not configured' };
...
async function callAnthropic(systemPrompt, userMessage, model, apiKey) {
    if (!apiKey)
        return { serviceName: null, error: 'Anthropic API key not configured' };
...
async function callGoogle(systemPrompt, userMessage, model, apiKey) {
    if (!apiKey)
        return { serviceName: null, error: 'Google API key not configured' };
