...
class Logger extends events_1.EventEmitter {
    ...
    `);
        ...
        try {
            this.db.exec('ALTER TABLE logs ADD COLUMN service_name TEXT');
        }
        catch { }
    }
    cleanup() {
    ...
    log(entry) {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(entry.id, entry.timestamp, entry.channel, entry.from, entry.senderName || null, entry.text || null, entry.type, entry.groupId || null, entry.groupName || null, entry.route || null, entry.responseText || null, entry.status, entry.latency ?? null, entry.sttTranscription || null, entry.ttsGenerated ? 1 : null, entry.formatApplied ? 1 : null, entry.formatOriginalText || null, entry.serviceName || null);
