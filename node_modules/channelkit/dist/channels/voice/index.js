...
exports.TwilioVoiceChannel = void 0;
const crypto_1 = require("crypto");
const twilio_1 = __importDefault(require("twilio"));
const base_1 = require("../base");
class TwilioVoiceChannel extends base_1.Channel {
    client;
    ...
    constructor(name, config) {
        ...
        this.client = (0, twilio_1.default)(config.account_sid, config.auth_token);
    }
    get cfg() {
        return this.config;
    }
    ...
    async connect() {
        try {
            ...
            console.warn(`⚠️  Could not verify Twilio number: ${err.message}`);
        }
        if (!this.publicUrl) {
    ...
    async setWebhookUrl(url) {
        try {
            ...
            this.connected = false;
            this.statusMessage = `Failed to configure Twilio webhook: ${err.message}`;
            this.emit('disconnected');
     ...
     */
    handleIncomingCall(payload, voiceConfig) {
        ...
        const sayOpts = { language };
        if (voiceName)
            sayOpts.voice = voiceName;
     ...
     */
    async handleRecording(payload, voiceConfig) {
        const callSid = payload.CallSid;
        ...
        try {
            ...
            unified._callSid = callSid;
            unified._voiceConfig = voiceConfig;
    ...
    handleRespond(callSid, voiceConfig) {
        ...
        if (response.media?.buffer && response.media.mimetype?.includes('audio')) {
            ...
            const audioId = this.storeAudio(Buffer.isBuffer(response.media.buffer) ? response.media.buffer : Buffer.from(response.media.buffer), response.media.mimetype || 'audio/mpeg');
...
exports.TwilioVoiceChannel = TwilioVoiceChannel;
