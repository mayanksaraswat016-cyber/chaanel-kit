...
class TelegramChannel extends base_1.Channel {
    bot = null;
    slashCommands = [];
    // userId → active service webhook (for multi-service routing)
    activeService = new Map();
    ...
    async connect() {
        ...
        if (this.slashCommands.length > 0) {
            await this.bot.api.setMyCommands(this.slashCommands.map(sc => ({
                command: sc.command,
        ...
        this.bot.on('message', async (ctx) => {
            ...
            const mediaDownloadTypes = ['audio', 'image', 'video', 'document', 'sticker'];
            if (mediaDownloadTypes.includes(unified.type)) {
                try {
                    let fileId;
                    let mimetype;
                    let filename;
            ...
            if (this.slashCommands.length > 0) {
                ...
                if (activeWebhook) {
                    unified._resolvedWebhook = activeWebhook;
                }
                else {
    ...
    async send(to, response) {
        ...
        if (response.media?.buffer && response.media.mimetype?.includes('audio')) {
            await this.bot.api.sendVoice(chatId, new grammy_1.InputFile(response.media.buffer, 'voice.ogg'));
            return;
        }
        if (response.text) {
    ...
    toUnified(ctx) {
        ...
        return {
            ...
            groupName: isGroup
                ? ('title' in msg.chat ? msg.chat.title : undefined)
                : undefined,
        };
    }
}
